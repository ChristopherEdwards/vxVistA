...
INPUTB(X) ;input tranfrom for NAME in BUILD file
 ...
 ;DSS/LM Begin mods
 I X["*" K:$P(X,"*",2,3)'?1.4N1"."1.2N.1(1"V",1"T").2N1"*"1.6N X Q
 S %=$L(X," ") I %<2 K X Q
 S %=$P(X," ",%) K:%'?1.4N1"."1.2N.1(1"V",1"T",1"B").2N X
 ;DSS/LM End mods
 Q
...
PCK(Y) ;check Package File name, Y=ien in package file
 ...
 ;DSS/LM Begin change - If BUILD name starts with PACKAGE prefix...
 N VFD S Y=$L($P(Z,U)),(%,VFD)=$P(^XPD(9.6,DA,0),U),%=$$PKG^XPDUTL(%)
 I $P(Z,U,2)=$E(VFD,1,$L($P(Z,U,2))) Q 1
 ;DSS/LM End change
