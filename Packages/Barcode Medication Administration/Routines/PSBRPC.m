...
SCANPT(RESULTS,PSBDATA) ; Lookup Pt by Full SSN
 ...
 ; 
 ;DSS/LM  - Begin insert for ID type override
 N VFD S VFD=$$GET^XPAR("SYS","VFD PSB SCANPT ID TYPE")
 S:VFD="" VFD="MRN" S $P(PSBDATA,U,3)=VFD
 ;DSS/LM  - End insert
 ;
 N DFN
 ;DSS/SGM - Begin MODS for MRN lookup
 S PSBDATA=$G(PSBDATA)
 I $P(PSBDATA,U,3)="MRN"!(PSBDATA'[U) D  I +$G(RESULTS(1)),RESULTS(1)<1 Q
 .Q:'$$VFIELD^DILFD(2,21600)
 .N VFDERR
 .S X=$$FIND1^DIC(2,"","",$P(PSBDATA,U),"VFD",,"VFDERR")
 .I X<1 S RESULTS(0)=1,RESULTS(1)="-1^Invalid Patient Lookup" Q
 .S (DFN,RESULTS(1),PSBDFN)=X
 .I $P(PSBDATA,U,3)="" S $P(PSBDATA,U,3)="MRN"
 .Q
 ;DSS/SGM - END MODS
