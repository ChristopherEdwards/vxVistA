VFDCDT ;DSS/SGM - DATE FUNCTION RPCS ;03/18/2005 15:31
 ...
 ;
CNVT(VFDC,INVAL,INTYPE,OUTYPE,OUTFMT,TIMEFMT,FUN) ;
 ; RPC: VFDC DATE CONVERT
 ...
 I $L(OUTYPE)>1 S VFDC=RET
 E  S X=$F("DFEMHG",OUTYPE)-1 S:X>0 VFDC=$P(RET,U,X)
 S:'$D(VFDC) VFDC="ERR"
OUT Q:+$G(FUN) VFDC
 Q
 ;
...
ETFM(X,FLAGS,LIMIT) ; convert external format to FM
 ...
 ;         if invalid return -1
 N Y,Z,DIERR,VFD,VFDER
 I $G(X)="" S X="NOW"
 S Z=$G(FLAGS)
 I Z="" S Z="ET"_$E("S",TIMEFMT=14)
 I X[":" S:Z'["T" Z=Z_"T" I $L(X,":")>2,Z'["S" S Z=Z_"S"
 S FLAGS=Z,LIMIT=$G(LIMIT) I $L(LIMIT),LIMIT'?7N.1".".N S LIMIT=""
 D DT^DILF(Z,X,.VFD,LIMIT,"VFDER") S Y=VFD
 I Y<1!(FLAGS'["E")!'$D(VFD(0)) Q Y
 Q Y_U_$P(VFD(0),":",1,$S(TIMEFMT=10:1,TIMEFMT=12:2,1:3))
 ;
