SISFIND ;SIS/LM - Find an object across Cache namespaces
 ;;1.0
 ;
 Q
FM(SISRSLT,SISFILE,SISENTRY,SISFLGS) ;[Public] Find entry in FileMan file
 ...
 I $G(SISFILE)>0,$L($G(SISENTRY)) N SISHOME,SISNMSP
 E  S SISRSLT(1)="-1^Invalid file or entry specification" Q
 S SISHOME=$ZU(5) N SISI,$ETRAP S $ETRAP="ZN SISHOME D ERR^SISFIND"
...
BLD(SISRSLT,SISCOM,SISENTRY,SISFLGS) ;[Public] Find entry in BUILD component
 ...
 ;
 I $L($G(SISENTRY)) S SISCOM=$G(SISCOM),SISFLGS=$G(SISFLGS)
 ...
 D WAIT^DICD W ! ;Could take a few minutes
...
FBLD(SISRSLT,SISCOM,SISENTRY,SIFLGS) ;[Private] Come here from BLD
 ...
 ; SISFLGS=[Optional]Flags
 ;
 N SISBIEN,SISBNM,SISCIEN,SISCNM,SISDIEN,SISX S SISBIEN=" "
...
FBLDCONT ;[Private] Common continuation of FBLD
 ...
 .S SISX=$G(^(SISDIEN,0)) Q:'($E($P(SISX,U),1,SISL)=SISENTRY)
...
ERR ;[Private] Error handler
 ...
 Q
DATA ;;List of namespaces to search
 ;;FOIA
 ;;LEX
 ;;VOS
 ;;VXKID
