VFDCFM06 ;DSS/SGM - FILEMAN DD UTILITIES ;10/21/2005 14:27
...
FIELD(VFDC,FILE,FIELD,FLAG,ATT,TYPE) ; RPC: VFDC FM GET FIELD ATTRIB
 ...
 I FLAG["Z",$$BROKER^VFDCUTL S FLAG=$TR(FLAG,"Z")
...
VFIELD(VFDC,FILE,FIELD,FUN) ; RPC: VFDC FM VERIFY FIELD
 ...
 S FILE=$$VFILE(,FILE,1)
 I FILE<0 S VFDC=FILE G OUT
 I FIELD="" D ERM(6) G OUT
 I +FIELD=FIELD S X=$$VFIELD^DILFD(FILE,FIELD) I X>0 S VFDC=FIELD G OUT
 S X=$$FLDNUM^DILFD(FILE,FIELD)
 I '$D(DIERR),X>0 S VFDC=X
 I $D(DIERR) S VFDC=$$ERR
 G OUT
 ;
...
ERR() ; if $D(DIERR) return "-1^"_error msg from VFDER or ^TMP("DIERR",$J)
 ...
 S VFDX="-1^"_$$MSG^VFDCFM01("VE",,,,INPUT) D CLEAN^DILF
 Q VFDX
 ;
...
INIT(STR) ; str - ^-delimited string of variable names to initialize
 ...
 I STR["FLAG",$G(FLAG)'="" S FLAG=$$CNVT^VFDCUTL(FLAG,"10NZ","U")
 I STR["TYPE" S TYPE=$S(TYPE=1:1,$$BROKER^VFDCUTL:0,TYPE="":1,1:TYPE)
 Q
