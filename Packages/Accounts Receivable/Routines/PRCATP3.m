PRCATP3 ; GENERATED FROM 'PRCA VENDOR PROFILE' PRINT TEMPLATE (#428) ; 03/30/98 ; (FILE 430, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(428,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
