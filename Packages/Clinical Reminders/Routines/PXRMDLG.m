PXRMDLG ; SLC/PJH - Reminder Dialog Edit/Inquiry ;07/29/2004
...
CHECK ;Search for sequence numbers with no dialog pointer
 ...
 S (SNUM,SEQ)=0
 F  S SEQ=$O(SEQTMP(SEQ)) Q:SEQ=""  S DCNT=0 F  S SNUM=$O(SEQTMP(SEQ,SNUM)) Q:+SNUM=0  S DCNT=DCNT+1 I DCNT>1 S DELTMP(DCNT)=SEQTMP(SEQ,SNUM) S DEL="Y"
...
XQORM ;Protocol Menu reset
 ...
 I $G(PXRMDIEN)'="",$P($G(^PXRMD(801.41,PXRMDIEN,100)),U,4)=1,'$G(PXRMINST) D
 .N FMENU
 .S FMENU=$O(^ORD(101,"B","PXRM DIALOG LOCK",0))_";ORD(101,"
 .I FMENU S XQORM("HIJACK")=FMENU
 Q
 ;
