PXRMEXID ;SLC/PJH - Reminder Dialog Exchange Install Routine.;08/16/2007
...
INSBLD(NAME,INAME) ;
 ...
 ..S REPL=$$CHKREPL^PXRMEXD1(NAME) I REPL>0 D INSREPL(NAME,REPL,.INAME)
 .S INAME(DNAME)=""
 .;Q:$$PXRM(DNAME)  S INAME(DNAME)=""
 .;Check for descendants
 .I $D(^TMP("PXRMEXTMP",$J,"DMAP",DNAME)) D INSBLD(DNAME,.INAME)
 Q
 ;Build list of replacement names
 ;-------------------------------
INSREPL(NAME,REPL,INAME) ;
 N DNAME,IDATA,ISEQ
 S ISEQ=0
 S IDATA=$G(^TMP("PXRMEXTMP",$J,"DREPL",REPL,NAME)) Q:IDATA=""
...
INSCOM(IND,SILENT) ;
 ...
 ..S INDS=IND
 ..N IDATA,INAME,IND
 ..I REPL>0 D INSREPL(PT01,REPL,.INAME)
...
INSSEL N ALL,IND,PXRMDONE,VALMY
 ...
 ;
 S IND=0
 F  S IND=$O(VALMY(IND)) Q:(+IND=0)!(PXRMDONE)  D INSCOM(IND,0)
 ;
 ;Clear locks
 D UNLOCK
 ;
