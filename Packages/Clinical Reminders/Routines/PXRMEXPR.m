PXRMEXPR ; SLC/PKR/PJH - Routines to create packed reminder definitions. ;12/12/2006
...
PACK(RTP,TMPIND) ;Create the packed reminder, store it in
 ...
 D DIALOG^PXRMEXDG(RIEN,.DLGLIST,.FINDLIST,.OBJLIST,.TEMLIST,.SPONLIST)
 ;
 ;If there were education topics make sure subtopics are included.
 D SUB^PXRMEXED(.FINDLIST)
 ;
