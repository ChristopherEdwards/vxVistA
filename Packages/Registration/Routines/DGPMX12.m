...
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,5) S:%]"" DE(1)=% S %=$P(%Z,U,6) S:%]"" DE(8)=% S %=$P(%Z,U,7) S:%]"" DE(9)=% S %=$P(%Z,U,10) S:%]"" DE(2)=%
 I $D(^(21600)) S %Z=^(21600) S %=$P(%Z,U,1) S:%]"" DE(5)=%
...
X1 I '$D(DGPMT) W !?3,*7,"USE BED CONTROL MOVEMENT OPTIONS!" K X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;10",DV="RFX",DU="",DLB="ADMIT REASON [SHORT]",DIFLD=.1
 G RE
...
X3 S VFDICD9=$$ADMITDX^VFDDGPM1
 Q
4 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=4 D X4 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X4 S:'(VFDICD9>0) Y="@216"
 Q
5 S DW="21600;1",DV="P80'",DU="",DLB="DIAGNOSIS [ICD]",DIFLD=21600.01
 S DU="ICD9("
 S X=VFDICD9
 S Y=X
...
X6 K VFDICD9
 Q
7 S DQ=8 ;@216
8 S DW="0;6",DV="R*P42'X",DU="",DLB="WARD LOCATION",DIFLD=.06
 S DE(DW)="C8^DGPMX12"
 S DU="DIC(42,"
 G RE
...
C8F1 Q
X8 S DIC("S")="I $S($D(^(""ORDER"")):^(""ORDER""),1:0)" D W^DGPMVDD I $D(X) D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D WARD^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;7",DV="*P405.4'X",DU="",DLB="ROOM-BED",DIFLD=.07
 S DE(DW)="C9^DGPMX12"
 S DU="DG(405.4,"
 G RE
...
C9F1 Q
X9 K:'$D(DGPMT) X I $D(X) S DIC("S")="I $D(^DG(405.4,""W"",+$P(^DGPM(DA,0),""^"",6),+Y)) D OCC^DGPMRB I 'DGPMOC" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D ROOM^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
