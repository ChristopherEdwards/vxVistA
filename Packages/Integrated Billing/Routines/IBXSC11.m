...
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,D=0 K DE(1) ;1
 S DIFLD=1,DGO="^IBXSC12",DC="3^2.01^.01^",DV="2.01MF",DW="0;1",DOW="ALIAS",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
...
X12 I $D(X) S:'$D(DPTX) DFN=DA D:'$D(^XUSEC("DG ELIGIBILITY",DUZ)) VAGE^DGLOCK:X="Y" I $D(X) D:$D(DFN) EV^DGLOCK
 Q
 ;
13 D:$D(DG)>9 F^DIE17,DE S DQ=13,DW=".36;1",DV="*P8'X",DU="",DLB="PRIMARY ELIGIBILITY CODE",DIFLD=.361
 S DE(DW)="C13^IBXSC11"
 S DU="DIC(8,"
 G RE
