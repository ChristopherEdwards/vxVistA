...
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^("M1")) S %Z=^("M1") S %=$P(%Z,U,2) S:%]"" DE(1)=%,DE(7)=% S %=$P(%Z,U,3) S:%]"" DE(19)=% S %=$P(%Z,U,10) S:%]"" DE(2)=%,DE(11)=%,DE(14)=% S %=$P(%Z,U,11) S:%]"" DE(20)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X5 S IBPSIDO=$P($G(^DGCR(399,DA,"M1")),U,2),IBPSQO=$P($G(^DGCR(399,DA,"M1")),U,10)
 Q
6 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=6 G A
7 S DW="M1;2",DV="FX",DU="",DLB="PRIMARY PROVIDER #",DIFLD=122
 S DE(DW)="C7^IBXSC36"
...
X8 I $G(IBPSQO)]"",X'=$G(IBPSID),X'=$G(IBPSIDO) S Y="@3214"
 Q
9 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=9 D X9 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X9 I $G(IBPSQO)="",$G(IBPSQUAL)="",X]"" S Y="@3214"
 Q
10 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=10 G A
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW="M1;10",DV="*P355.97'",DU="",DLB="PRIMARY ID QUALIFIER",DIFLD=128
 S DU="IBE(355.97,"
...
X12 S Y="@3215"
 Q
13 S DQ=14 ;@3214
14 S DW="M1;10",DV="*P355.97'",DU="",DLB="Primary ID Qualifier",DIFLD=128
 S DU="IBE(355.97,"
 G RE
...
X18 I '$$SUPPPT^IBCEP7B(DA,2) S Y="@3222"
 Q
19 S DW="M1;3",DV="FX",DU="",DLB="SECONDARY PROVIDER #",DIFLD=123
 S DE(DW)="C19^IBXSC36"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
X19 K:$L(X)>13!($L(X)<3)!($TR(X," ")="") X
 ...
 Q
 ;
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW="M1;11",DV="*P355.97'",DU="",DLB="SECONDARY ID QUALIFIER",DIFLD=129
 S DU="IBE(355.97,"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
X21 W !,"No Billing Prov Secondary IDs will be sent for Secondary Ins/Elect Plan Type."
 Q
...
X29 S IBPSDAT=$$GETID^IBCEP7B(DA,2),IBPSID=$P(IBPSDAT,U),IBPSQUAL=$P(IBPSDAT,U,2) I IBPSID="" S Y="@322"
 Q
30 D:$D(DG)>9 F^DIE17 G ^IBXSC37
